...
def main():
    ...
    print(cirr_val_dataset.__len__())
...
def qdrant_index(option= 0, vector_idx = 0):

    print(option, vector_idx)
    if option in [0,1]:
        cirr_pt_file = [f for f in os.listdir(data_path) if 'cirr' in f and 'pt' in f]
        for f in tqdm(cirr_pt_file,  desc="Processing CIRR files: "):
            ...
            for idx, row in tqdm(enumerate(cirr_index_feature), desc=f"Inserting CIRR {op} file: ", total=len(cirr_index_feature)):
                ...
                try:
                    sub_path = os.path.join('cirr_dataset',op,cirr_index_name[idx]+'.png')
                    img_path = os.path.join(server_base_path,sub_path)
                ...
                try:
                    ...
                    with open("insert_feature_log.txt",'a') as file:    
                        file.write(f"{vector_idx}\t{sub_path}\n")
                vector_idx += 1
    ...
    if option in [0,2]:
        ...
        for f in tqdm(fashion_pt_file, desc="Processing FashionIQ files"):
            ...
            for idx, row in tqdm(enumerate(fashion_index_feature), desc=f"Inserting FashionIQ {dress_type[0]} file: ", total=len(fashion_index_feature)):
                imgdata = ImageData(id= vector_idx,
                                    index_date=datetime.now(),
                                    image_vector=row,
                                    ...
                                    dataset='fashionIQ')
                try:
                    sub_path = os.path.join('fashionIQ_dataset','images',fashion_index_name[idx]+'.png')
